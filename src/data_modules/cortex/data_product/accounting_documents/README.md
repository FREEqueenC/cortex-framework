...
The Accounting Documents data product provides clean, analytics-ready financial transaction models from SAP ERP accounting document headers and segments. It enables high-fidelity financial reporting, auditing, and general ledger line item analytics for enterprise finance teams.
...
| :--- | :--- | :--- |
| `accounting_documents_header` | Captures accounting document header metadata from SAP BKPF (e.g., posting dates, entry dates, user details, document status, and currency keys), enriched with calendar date dimensions for financial reporting. | `SAP ECC` / `SAP S/4HANA` |
| `accounting_documents_item` | Captures detailed accounting document line items from SAP BSEG (e.g., amounts, taxes, accounts, and order details) with dynamically adjusted currency decimal shifts, enriched with calendar date dimensions. | `SAP ECC` / `SAP S/4HANA` |
