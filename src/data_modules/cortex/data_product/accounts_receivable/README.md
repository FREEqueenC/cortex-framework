...
Consolidates customer accounting transactions, open invoices, and cleared items into a standardized reporting layer. This empowers treasury teams and analytics platforms with insights into DSO (Days Sales Outstanding), aging analyses, and credit-risk modeling.
...
| :--- | :--- | :--- |
| `accounts_receivable` | Provides a unified view of accounts receivable from the SAP S/4HANA universal ledger journal entries (ACDOCA, BKPF, BSEG), filtered by Customer account type to support DSO (Days Sales Outstanding) analysis, aging reporting, and credit-risk modeling. | `SAP S/4HANA` |
| `customer_open_items` | Provides a detailed view of open customer transaction line items from SAP ECC (BSID) to support operational tracking and outstanding invoice analysis. | `SAP ECC` |
| `customer_cleared_items` | Provides a detailed view of cleared customer transaction line items from SAP ECC (BSAD) to support historical reconciliation and customer payment auditing. | `SAP ECC` |
