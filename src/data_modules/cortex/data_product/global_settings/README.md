...
Establishes enterprise-wide foundational standards, incorporating countries, currencies, and languages. It provides the essential cross-domain reference layers needed for accurate localized operations, global tax calculation, and transaction mapping.
...
| :--- | :--- | :--- |
| `company_codes` | Establishes the core enterprise structures by providing company definitions from SAP T001, detailing organizational locations, base currency identifiers, charts of accounts, and active fiscal year variants. | `SAP ECC` / `SAP S/4HANA` |
| `languages` | Provides a system-wide language dictionary from SAP T002 and T002T mapping language keys to standardized names to support multilingual localized reporting settings. | `SAP ECC` / `SAP S/4HANA` |
| `countries` | Offers country-specific master records from SAP T005 and T005T containing localized names, formatting standards, regional settings, and international code mappings to enable international trading analytics. | `SAP ECC` / `SAP S/4HANA` |
| `country_dialing_codes` | Captures international and country-specific telephone and fax dialing codes from SAP T005K to enable telecommunication metadata lookup. | `SAP ECC` / `SAP S/4HANA` |
| `country_tax_regions` | Stores state, provincial, and regional master details from SAP T005S and T005U alongside language-specific descriptions to enable regional tax key lookups. | `SAP ECC` / `SAP S/4HANA` |
| `system_status_texts` | Captures language-dependent status texts for system statuses from SAP TJ02T to enable localized status mappings. | `SAP ECC` / `SAP S/4HANA` |
