...
Provides a unified view of accounts payable across legacy open items, cleared transactions, and universal ledger journal entries. It covers core financial domains including vendor ledger accounting, payment terms optimization, and liability tracking, supporting enterprise cash-flow analytics and automated reconciliation pipelines.
...
| :--- | :--- | :--- |
| `accounts_payable` | Provides a unified view of accounts payable from the SAP S/4HANA universal ledger journal entries (ACDOCA, BKPF, BSEG), filtered by Vendor account type to support cash-flow analytics and liability tracking. | `SAP S/4HANA` |
| `vendor_open_items` | Provides a detailed view of open vendor transaction line items from SAP ECC (BSIK) to support operational tracking and outstanding liability analysis. | `SAP ECC` |
| `vendor_cleared_items` | Provides a detailed view of cleared vendor transaction line items from SAP ECC (BSAK) to support automated reconciliation and historical payment auditing. | `SAP ECC` |
