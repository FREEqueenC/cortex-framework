...
The Condition Contracts data product models SAP Condition Contract Settlement (CCS) master data, settlement calendars, business volume details, and detailed settlement statements across ECC and S/4HANA systems.
...
| :--- | :--- | :--- |
| `condition_contracts_header` | Captures header-level SAP Condition Contract Settlement (CCS) definitions from WCOCOH (e.g., partner numbers, contract types, and validity dates) to track commercial rebate contracts. | `SAP ECC` / `SAP S/4HANA` |
| `condition_contracts_item` | Captures line-item SAP Condition Contract Settlement rules from WCOCOI (e.g., eligibility criteria, materials, and settlement parameters) to define contract settlement conditions. | `SAP ECC` / `SAP S/4HANA` |
| `rebates_settlement_calendar` | Captures scheduled settlement calendar dates and settlement document references from WB2_D_SETTL_CAL to monitor payout status and evaluation frequencies. | `SAP ECC` / `SAP S/4HANA` |
| `condition_contract_settlement_detailed_statement` | Provides a detailed settlement evaluation statement from WB2_D_BVDETAIL, reflecting business volume and settlement amounts, to support commercial rebate audits. | `SAP ECC` / `SAP S/4HANA` |
