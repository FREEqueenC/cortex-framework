...
The Fiscal Year Variants data product provides clean financial calendar modeling assets for SAP Fiscal Year Variants (`T009`) and their associated period definitions (`T009B`) across ECC and S/4HANA systems.
...
| :--- | :--- | :--- |
| `fiscal_year_variants` | Provides master definitions of SAP Fiscal Year Variants from T009, including calendar year markers, year-dependent indicators, and the number of posting and special periods. | `SAP ECC` / `SAP S/4HANA` |
| `fiscal_year_variant_periods` | Provides period definitions from SAP T009B, mapping calendar months and days to corporate fiscal posting periods and periods shifts. | `SAP ECC` / `SAP S/4HANA` |
