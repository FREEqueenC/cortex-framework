...
from data_modules.cortex.data_foundation.sap import metadata_provider
        ...
        *,
        ...
        if table_settings_file and table_settings_file.exists():
            try:
                ...
                raise ValueError(f"Invalid table settings file for {module_id}") from e
        else:
        ...
        if not source_config:
            logger.error("Data source %s not found in config.", module_config.data_source_id)
            return
...
_SQLX_TEMPLATE = jinja2.Template(
  ...
  type: "operations",
  hasOutput: true,
  database: {{ module_config_js }}.targetProjectId,
  schema: {{ module_config_js }}.targetDatasetId,
  name: "{{ target_name }}",
  tags: {{ tags | tojson }}
}
...
{{ merger_macro }}(ctx, {
  ...
  target_ref: self(),
  source_ref: ref({{ module_config_js }}.sourceDatasetId, "{{ base_table }}")
  {% if partition %}
...
def _render_data_foundation_sqlx(
    ...
    target_obj = table_config.target
    tags = target_obj.tags or []

    target_name = (target_obj.table_name or base_table).lower()
    module_config_js = f"config.foundation.{module_id}"
    ...
    return _SQLX_TEMPLATE.render(
        module_config_js=module_config_js,
